...
export default function FollowingPage() {
  ...
  useEffect(() => {
    if (user) {
      api.get('/profile/following')
        ...
        .catch(err => {
          setError('Không thể tải danh sách người dùng đang theo dõi.');
          console.error(err);
        })
        .finally(() => {
          setLoading(false);
  ...
  return (
    <div className="container">
      <h1 className="text-3xl font-bold text-white mb-6">Đang theo dõi</h1>
      ...
      {following.length > 0 ? (
        ...
        <p className="text-slate-400">Bạn chưa theo dõi người dùng nào.</p>
      )}
    </div>
  );
}